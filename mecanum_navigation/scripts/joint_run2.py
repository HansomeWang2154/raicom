...
class MultiGoalPublisher:
    def __init__(self):
        ...
        # 定义目标位置（在最后一个点前插入两个新点）
        self.goal_list = [
            (1.5, 0.2, 0.0),     # 目标点1 开机就发布，到达或超时后直接发布目标点2
            (0.85, 0.6, 0.0),   # 目标点2 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (2.21, 0.55, 0.0),   # 目标点3 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (3.5, 0.45, 0.0),    # 目标点4 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (5.0, 0.40, 0.0),    # 目标点5 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (6.23, -0.30, 0.0),  # 目标点6 到达或超时后直接发布目标点7
            (6.30, 1.03, 0.0),   # 目标点7 到达或超时后直接发布目标点8
            (6.30, 2.39, 0.0),   # 目标点8 到达或超时后直接发布目标点9
            (4.0, 2.5, 0.0),     # 目标点9 到达后立即发布下一个
            (1.0, 2.3, 0.0),     # 目标点10 到达后立即发布下一个
            (0.45, 3.7, 0.0),    # 目标点11 到达或超时后向/wheels/arrive_box发布消息，延时5后发布目标点12
            (0.92, 2.20, 0.0),   # 目标点12 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (2.21, 2.10, 0.0),   # 目标点13 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (3.8, 2.00, 0.0),    # 目标点14 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (5.5, 1.90, 0.0),    # 目标点15 到达或超时后向/wheels/arrive发布消息，并等待/yolo/picked话题
            (4.0, 2.5, 0.0),     # 新插入目标点16 - 到达后立即发布下一个
            (1.0, 2.3, 0.0),     # 新插入目标点17 - 到达后立即发布下一个
            (0.45, 3.7, 0.0)     # 原目标点16变为18 到达或超时后向/wheels/arrive_box发布消息，延时5后结束程序
        ]
    ...
    def goal_done_cb(self, status, result):
        with self.state_lock:
            ...
            # 根据目标点序号执行不同操作
            if self.current_goal_index in [1, 2, 3, 4, 14, 11, 12, 13]:  # 需要等待YOLO的目标点
            ...
            elif self.current_goal_index in [8, 9, 15, 16]:  # 需要立即发布下一个目标点的点
                rospy.loginfo("目标点#{self.current_goal_index+1}完成，立即发布下一个目标")
            
            elif self.current_goal_index in [10,17]:  # 最后一个点
    ...
    def timeout_cb(self, event):
        with self.state_lock:
            ...
            # 根据目标点序号执行不同操作
            if self.current_goal_index in [1, 2, 3, 4, 10, 11, 12, 13]:  # 需要等待YOLO的目标点
                rospy.loginfo("发布到达消息")
                self.arrived_pub.publish(Int32(1))
            ...
            elif self.current_goal_index == 17:  # 最后一个点
                rospy.loginfo("发布到达box消息")
                self.arrived_box_pub.publish(Int32(1))
                rospy.sleep(5.0)  # 延时5秒
